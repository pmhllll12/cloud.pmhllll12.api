"""James — Neon PostgreSQL: `PersonCommand` / `BookingCommand` 적재."""
...
class JamesDirectorPgRepository(JamesRepository):
    ...
    async def receive_uploaded_records(
        ...
        try:
            # `AsyncSession.run_sync` 콜백 인자는 동기 Connection 이 아닐 수 있어
            # `create_all` 은 별도 `AsyncConnection.run_sync` 경로로 실행한다.
