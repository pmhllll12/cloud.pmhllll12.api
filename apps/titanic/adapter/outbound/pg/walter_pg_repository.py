...
async def ingest_persist_payload(
    ...
    """`walter_repository.WalterPersistPayload` → Neon `titanic_walter_passengers`."""
    rows = payload.rows
    if not rows:
        logger.info("[walter_pg_repository] 저장 스킵 — rows=0 filename=%s", payload.filename)
        return 0

    logger.info(
        "[walter_pg_repository] Neon 저장 시작 — filename=%s rows=%s columns=%s",
        payload.filename,
        len(rows),
    ...
    try:
        ...
        logger.exception("[walter_pg_repository] Neon 저장 실패 — filename=%s", payload.filename)
        raise ValueError(f"Neon DB 저장에 실패했습니다: {exc}") from exc

    logger.info(
        "[walter_pg_repository] Neon commit 완료 — titanic_walter_passengers %s행 (filename=%s)",
        len(rows),
        payload.filename,
    )
    return len(rows)
...
async def ingest_fetch_all_passengers(session: AsyncSession) -> list[dict[str, Any]]:
    """Neon → API 행 형식."""
    logger.info("[walter_pg_repository] Neon 조회 시작 — titanic_walter_passengers")
    ...
    logger.info("[walter_pg_repository] Neon 조회 완료 — %s행", len(rows))
    return rows


async def receive_persist_from_port(
    repository: "WalterPgRepository",
    payload: WalterPersistPayload,
) -> int:
    """출력 포트 `submit_persist_upload` → PG 어댑터 수신."""
    logger.info(
        "[walter_pg_repository] 수신 — persist filename=%s rows=%s",
        payload.filename,
        len(payload.rows),
    )
...
async def receive_fetch_from_port(repository: "WalterPgRepository") -> list[dict[str, Any]]:
    """출력 포트 `submit_fetch_all_passengers` → PG 어댑터 수신."""
    logger.info("[walter_pg_repository] 수신 — fetch_all")
    return await repository.load_all_rows()


class WalterPgRepository(WalterRepositoryPort):
    """`WalterRepositoryPort` 구현 — Neon PostgreSQL."""

    def __init__(self, session: AsyncSession) -> None:
        self._session = session
        logger.info("[walter_pg_repository] 인스턴스 생성 — session=%s", id(session))

    async def persist_payload(self, payload: WalterPersistPayload) -> int:
        logger.info(
            "[walter_pg_repository] persist_payload — filename=%s → ingest_persist_payload",
            payload.filename,
        )
        return await ingest_persist_payload(self._session, payload)

    async def load_all_rows(self) -> list[dict[str, Any]]:
        logger.info("[walter_pg_repository] load_all_rows → ingest_fetch_all_passengers")
    ...
    async def save_upload(
        ...
        *,
        ...
        logger.info(
            "[walter_pg_repository] save_upload — filename=%s rows=%s → persist_payload",
            filename,
            len(rows),
        )
    ...
    async def fetch_all(self) -> list[dict[str, Any]]:
        logger.info("[walter_pg_repository] fetch_all → load_all_rows")
