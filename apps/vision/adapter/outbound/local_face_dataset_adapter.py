...
_DEFAULT_DATASET_DIR = Path(__file__).resolve().parents[2] / "resources" / "yolo_train"


class LocalFaceDatasetAdapter(FaceDatasetPort):
    """로컬 디렉터리(`apps/vision/resources/yolo_train`)에서 인물별 얼굴 분류 데이터셋을 읽는다.

    나중에 S3 등 다른 소스로 바꾸더라도 FaceDatasetPort를 구현하는 어댑터만 교체하면 되고,
    FaceTrainingInteractor(App 영역)는 변경할 필요가 없다.
    """
