...
def test_blank_filename_raises():
    with pytest.raises(ValueError, match="filename은 비어 있을 수 없습니다"):
        AnalyzedImage(filename="   ", caption="설명", tags=[], analyzed_at=datetime.now())


def test_blank_caption_raises():
    with pytest.raises(ValueError, match="caption은 비어 있을 수 없습니다"):
        AnalyzedImage(filename="cat.jpg", caption="  ", tags=[], analyzed_at=datetime.now())


def test_immutable():
    image = AnalyzedImage(
        filename="cat.jpg", caption="설명", tags=[], analyzed_at=datetime.now()
    )
